...
class Cancel_order_model extends CI_Model
{
	...
	public function is_product_exist($data, $app_master_id='', $userid)
	{
		...
		$this->db->group_start();
            ...
            $this->db->where_in('item.product_id',$data['product_id']);            
		$this->db->group_end();
        $this->db->group_start();
		...
		{
			$this->db->select('count(product_id) as total_item,item_id,order_id');
			...
			{
				$p_status = $this->cancel_product($data['product_id'],$master_id);
				$o_status = $this->cancel_whole_order($userid,$master_id);
                $s_status = $this->cancel_seller_order($userid,$query2->row()->order_id);
				$i_status = $this->insert_cancellation_reason($userid,$query2->row()->item_id,$master_id,$query2->row()->order_id,$data['reason'],$data['comment']);
		        $insert_buyer_array = array('master_order_id'=>$master_id,'order_status'=>7,'status_date'=>$this->date);
                $t_status = $this->insert_recored_in_tracking_table($insert_buyer_array,'tblblk_buyer_order_tracking');
                $insert_seller_array = array('order_id'=>$query2->row()->order_id,'order_status'=>7,'status_date'=>$this->date);
                $t_status = $this->insert_recored_in_tracking_table($insert_seller_array,'tblblk_seller_order_tracking');
				if($p_status == true && $o_status == true && $i_status == true && $t_status == true && $s_status == true)
					return 1;
				else
					return 0;
			}
			else
			{
				$this->db->select('count(product_id) as total_item,item_id,order_id');
    			$this->db->from('tblblk_order_items');
                $this->db->where(array('order_id'=>$query2->row()->order_id,'order_item_status'=>1));                    			
    			$query3 = $this->db->get();
                
                if($query3->row()->total_item == 1 && $query3->row()->total_item != 0)
                {
    		        $insert_array = array('order_id'=>$query3->row()->order_id,'order_status'=>7,'status_date'=>$this->date);
                    $t_status = $this->insert_recored_in_tracking_table($insert_array,'tblblk_seller_order_tracking');
    				$i_status = $this->insert_cancellation_reason($userid,$query3->row()->item_id,$master_id,$query3->row()->order_id,$data['reason'],$data['comment']);
    				$p_status = $this->cancel_product($data['product_id'],$master_id);
    				$o_status = $this->cancel_seller_order($userid,$query3->row()->order_id);
                    if($p_status == true && $o_status == true && $i_status == true)
    					return 1;
    				else
    					return 0;
                }
                else
                {
    				$i_status = $this->insert_cancellation_reason($userid,$query3->row()->item_id,$master_id,$query3->row()->order_id,$data['reason'],$data['comment']);
    				$p_status = $this->cancel_product($data['product_id'],$master_id);
                    if($p_status == true && $i_status == true)
    					return 1;
    				else
    					return 0;
                }
	...
	public function cancel_product($product_id,$master_order_id)
	{
    ...
    public function cancel_seller_order($user_id,$order_id)
	{
		$this->db->trans_start();// Starting transaction
		$update = array('order_status'=>7);
		$this->db->where(array('user_id'=>$user_id,'order_id'=>$order_id));
		$this->db->update('tblblk_product_orders',$update);
		$this->db->trans_complete();// transaction ends here
		if($this->db->trans_status()===FALSE){
			return false;
		}
	...
	public function insert_cancellation_reason($userid,$item_id,$master_order_id,$order_id,$reason_id,$comment)
	{
		...
		$this->db->insert('tblblk_orders_cancellation',$insert);
		$this->db->trans_complete();// transaction ends here
		if($this->db->trans_status()===FALSE)
			return false;
		else
	...
	{
        $this->db->select('count("product_id") AS total_items,order_id');
        $this->db->from('tblblk_order_items');
        $this->db->where(array('master_order_id'=>$master_order_id,'order_item_status'=>1));
        $query1 = $this->db->get();
	...
	{
		$this->db->select('pro_order.unique_order_id As order_id,order.master_order_id,order.payment_type,order.exp_order_date,item.piece_per_set,item.product_id,product.set_description,item.price,item.order_item_status,order.order_status,track.order_status as track_status,track.status_date,product.item_name,product.shipping_time, order.order_date, order.master_order_id, item.quantity,image.image_name, payment.total_amount');
    ...
    public function get_seller_user_mobile($data)
    {
        ...
        $this->db->where_in('product.product_id',$data['product_id']);
        $query = $this->db->get();
