    ...
    Private function compiled_query($select, $category = '', $sub_category ='',$subtosub_category = '', $brand = '', $product_key_word = '', $price = '', $color = '', $size = '', $discount = '', $sets = '', $credit_days = '', $group_by = true,$sort_by = '')
    {
	    ...
	    if($product_key_word !='')
	    {
		    ...
		    if(preg_match('/^men/', trim($product_key_word)))
		    {
                ...
                $this->db->or_where("FIND_IN_SET('".$this->db->escape_str(trim($url))."', product_keywords.keyword_url)");
                $this->db->or_where("FIND_IN_SET('".$this->db->escape_str(trim($product_key_word))."', product_keywords.product_keyword)");
                $this->db->or_where("FIND_IN_SET('".$this->db->escape_str(trim($product_key_word))."', category.search_keywords)");
                $this->db->or_where("FIND_IN_SET('".$this->db->escape_str(trim($product_key_word))."', sub_category.search_keywords)");
                $this->db->or_where("FIND_IN_SET('".$this->db->escape_str(trim($product_key_word))."', subtosub_category.search_keywords)");
                $this->db->or_where("FIND_IN_SET('".$this->db->escape_str(trim($product_key_word))."', product_keywords.product_meta_keyword)");
