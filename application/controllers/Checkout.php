...
{   
    ...
    {
        $data = $this->data;
        $json = $product_ids = $product_qtys = array();
        $shipping_charge = 0;
        $delvry_id = $this->input->post('delvry_address_id');
        $data['user'] = $this->session->userdata('user_id');
        $addrs = $this->checkout_model->GetDeliveryAddress($data['user'],$delvry_id);
        $user_wallet = $this->checkout_model->getUserWallet($data['user']);
        $pincode_status = checkPincodeAvailability($addrs->pincode);
        $cart_info 	  = create_cart_product_listing($data['cart_info']);
        foreach ($cart_info['cart_list'] as $key => $value) {
            if(get_product_stocks(array('product_id'=>$value->id,'product_url'=>$value->product_url)) > 0) 
            {
                $product_ids[]  = $value->id;
                $product_qtys[] = $value->qty;
            } 
        }        
        $shipping_charge = get_logistic_charges($product_ids,$delvry_id,$product_qtys)['shipping_charges'];
        ...
        {   
            if(!empty($addrs))
            {
                ...
                if(!empty($cart_info))
                {
                    if(!empty($cart_info['cart_totals']))
                    {
                        if($shipping_charge > 0)
                            $json['shipping_charge'] = $shipping_charge;
                       
                        if($cart_info['cart_totals']['sub_total'] > 0)
                            $json['sub_total'] = $cart_info['cart_totals']['sub_total'];

                        if($cart_info['cart_totals']['total_vat'] > 0)
                            $json['total_vat'] = number_format((float)$cart_info['cart_totals']['total_vat'],2,'.','');

                        if($cart_info['cart_totals']['voucher_total'] > 0)
                            $json['voucher_total'] = number_format((float)$cart_info['cart_totals']['voucher_total'],2,'.','');

                        $json['round_off'] = number_format((float)round($cart_info['cart_totals']['cart_total']) - $cart_info['cart_totals']['cart_total'],2,'.','');

                        $json['final_price'] = round($cart_info['cart_totals']['cart_total'] + $shipping_charge);

                        /*if(empty($user_wallet))
                        {
                            
                        }
                        else if($cart_info['cart_totals']['cart_total'] - $user_wallet > 0)
                        {
                            $json['final_price'] = round($cart_info['cart_totals']['cart_total'] + $shipping_charge - $user_wallet);
                        }
                        else
                        {   
                            $json['final_price'] = 0;
        ...
        {
            $json['pincode_status'] = 'Sorry! Delivery Not Available in Your Area.';
        }
        echo json_encode($json);
    }
    ...
    {   
        $data = $this->data;
        $product_ids = $product_qtys = array();
        $shipping_charges = 0;
        $logistic_id = 1;        
        ...
        {   
            if(get_product_stocks(array('product_id'=>$value->product_id,'product_url'=>$value->product_url)) > 0) 
            {
                ...
                $objcate->size_name = empty($value->size_name) ? '' : $value->size_name;
        ...
        $shipping_charges = get_logistic_charges($product_ids,$delivery_id,$product_qtys)['shipping_charges'];                        
    ...
    public function getRazorKey()
    {
        ...
        $shipping_charge = get_logistic_charges($product_ids,$post['delivery_id'],$product_qtys)['shipping_charges'];        
        ...
        if($this->input->post())
        {
            ...
            if($post['user_id'] == $user_id)
            {
                ...
                $json['amount']     = (strtolower(trim($post['is_wallet'])) == 'yes')?$this->create_user_amount_with_bulk_wallet($user_id,$shipping_charge):round($cart_info['cart_totals']['cart_total'] + $shipping_charge);
                echo json_encode($json);
            }
            else
            {
