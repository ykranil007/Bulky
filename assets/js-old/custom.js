...
/* accordion main menu mobile  */
     ...
     $('.acc_menu li a').click(function(ev) {
		...
		if($(this).next('.sub_menu').css('display')=='block'){
...
$('.ac_navbar > h4').click(function()
{
	...
	var windowWidth = $(window).width();
	 if (windowWidth <= 999) 
	 {
		 $(".navouter").slideToggle('fast', function(){ that.toggleClass('active');});
	 }
	 else
	 {
		 $(this).removeClass('active');
...
$(".logsign").fancybox({
    ...
    'afterClose': function() {
        ...
        $('#login_email,#login_password,#firstname,#email,#password,#confirmpassword,#mobile').val('');

        $('#login_email_error,#login_password_error,#firstname_error,#email_error,#password_error,#confirmpassword_error,#mobile_error,#terms_error').hide();
...
$("#userUpdateButton").click(function(){
...

	{
		...
		$('#user_error_info').show().text('please Enter First Name');
		...
		$.ajax({
...

			{
...

				{
					...
					$("#SuccessMsg").text(json['error_msg']);
