jQuery(function($) {
    ...
    function create_user_request()
    {
        ...
        $('input[id^="set-"]:checked').each(function (){sets.push($(this).attr('url'));});
        $('input[id^="credit_days-"]:checked').each(function (){credit_days.push($(this).attr('url'));});    
        if(window.location.search)
        {
            ...
            if(get_parameter_by_name('search', query_string)!='' || get_parameter_by_name('search', query_string)!= null)
            {
                ...
                if(keywords != '' && keywords != null)
                {
                    ...
                    //product_key_word = replacer(keywords);
                     product_key_word= keywords;
                }
                //alert(product_key_word);
            }
        }
        ...
        get_product_list(category, sub_category, sub_tosub_category, brands, product_key_word, price, color, size, discount, sets, credit_days, page_no, sort_by);
    }
    
    function get_product_list(category, sub_category, sub_tosub_category, brands, product_key_word, price, color, size, discount, sets, credit_days, page_no,sort_by) 
    {
         $.ajax({
            ...
            {            
                ...
                { 
                   ...
                   $('.sets_info').html(json.sets_info.html);
                   //$('.credit_days_info').html(json.credit_days_info.html);

                   if(parseInt(json.total_product) > 0)
                   {
                      //console.log(json);
                      click_more();
                      $('.left_sidebar').show();
                      $('.product_list').html(json.products_listing);
                      stop_loader(json.total_product,24,page_no);
                      $("html, body").animate({ scrollTop: 80 }, "slow");
                      //alert($('.product_list > ul > li').length);
                      $("img.lazy").lazyload({event : "sporty"});
                      setTimeout(function() { $("img.lazy").trigger("sporty") }, 1000);
                   }
                   else
                   {
                     $('.breadcrumbs').after(json.products_listing); 
                     $('.product_list, .pagination_container').html('');
                     $('.left_sidebar,.main_slider,.catalog_product_header').remove();
                   }          
                }
            },
            error: function(data) { console.log(data);}
            //error: function(xhr,textStatus,error){console.log(error+textStatus);}
        });
    }   
    ...
    $('.sub_tosub_categorylist').click(function(){$("label > input[type='radio']").click(function (){create_default_page();create_user_request();});});
    ...
    $('.sets_info').click(function(){$(".checkbox > label > input[type='checkbox']").click(function (){
        create_default_page();create_user_request();});});
    $('.credit_days_info').click(function(){$(".checkbox > label > input[type='checkbox']").click(function (){create_default_page();create_user_request();});});
    ...
    function click_more()
    {
        $('.more').on('click', function(e){e.preventDefault();$(this).next('div[class="panel_more"]').addClass('open');});
