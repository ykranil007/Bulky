...
{ 
	...
	{
		$('tbody[data-name="get_records"]').html('');
	...
	{ 
		...
		getProductListing(page_no,product_status);
	});
	
	function getProductListing(page_no=1,product_status)
	{ 
		$.ajax({
			url			: "pending/product/listing",
			type 		: 'post',
			data		: {page:page_no,product_status:product_status},
			dataType 	: 'JSON',
			beforeSend	: function()
			{
				$('tbody[data-name="get_records"]').html('');
			...
			success	: function(response) 
			{
				...
				if(response.count > 0)
				{
					$('tbody[data-name="get_records"]').html(response.html);
				}
				else
				{
				  $('tbody[data-name="get_records"]').html(' <tr class="text-center text-danger"><td colspan="4">No Record Found</td></tr>');
				}
