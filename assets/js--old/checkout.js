...
$('.wallet_radio').click(function(){
    ...
    var fnl_price = (tot_price + delivery_charge + tax_charge);
...
$( "#rzp-button1,#wallet_pay_btn" ).click(function() {
  ...
  $.ajax({
  		...
  		{
  			if(json.amount > 0){
  				razorpay_api(json.razor_key,json.user_info,json.amount,json.master_id);
  			}
  			else{
...
function razorpay_api(key,user_info,amount,master_id)
{
	  var wallet_status = '';  
	  if($('input[name=radio]:checked').is(':checked'))
	  {
	  ...
	  var options = {
	    ...
	    "handler": function (response){
	        ...
	        $.ajax(
			{
				...
				type	: 'POST',
				data	: {razorpay_payment_id:response.razorpay_payment_id,delivery_id:delivery_id,wallet_status:wallet_status,master_id:master_id},
				dataType: 'json',
				beforeSend : function() 
				{
					$("#confirm_order_btn").html("<img src='assets/images/load.gif' width='18' style='margin-top:12px; margin-right:4px;' align='left' /> Please Wait...");
