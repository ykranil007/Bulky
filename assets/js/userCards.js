...
//=========VALIDATE ADD CARED FIELDS =======================================
	$("#save_cards_details").on('click',function(){
		...
		{	
			...
			else if( entered_year < currentYear )
			{
				card_status = false;
				$('#card_year').closest('div').addClass('has-error');
				$('#error_msg').show();
			...
			else
			{
				...
				if(card_status == true)
				{
					...
					$.ajax({
							...
							{
								if(json.card_exist > 0)
								{
									$('#cardNo').closest('div').addClass('has-error');
									$('#error_msg').show().text('Card No. is already exist.');
								}
								else
								{
								$('#cardNo').closest('div').removeClass('has-error');
								$('#error_msg').hide();
								$("#add_cards_div").hide();
								location.reload();
								}
...
/*$('.financial_signup').click(function(){
    alert();
   $("#financial_signup_popup").trigger( "click" ); 
});*/

/*$(function() {
  $("#financial_signup_popup").fancybox({
    'zoomSpeedIn': 300,
    'zoomSpeedOut': 300,
    'overlayShow': false
  }); 
...
$('#fn_get_started').click(function(){
   $('.get_start').hide();
   $('#user_section').show();
});

/*$('.buyer_started').click(function(){
   $.fancybox.close();
