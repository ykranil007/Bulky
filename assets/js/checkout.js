	...
	$.ajax(
	{
		...
		{ 
			...
			{				
				...
				$('#show_delivery_charge').html('&#8377; ' + json['shipping_charge']);
				$('#sub_total').html('&#8377; ' + json['sub_total']);
				$('#total_tax').html('&#8377; ' + json['total_vat']);
				if(json['total_vat']){
					$('#total_tax_val').show();
					$('#total_tax').html('&#8377; ' + json['total_vat']);
				}
				if(json['voucher_total']){
					$('#voucher_total').show();
					$('#voucher_value').html('&#8377; - ' + json['voucher_total']);
				}
				$('#round_off').html('&#8377; ' + json['round_off']);
				$('#pro_final_price').html('&#8377; ' + json['final_price']);
